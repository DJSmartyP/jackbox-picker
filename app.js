...
function winnerHTML(g){const p=packInfo(g.pack);return `<div class="winner-showcase"><div class="winner-art"><img class="winner-art-rotated-img" src="${esc(gameArtUrl(g))}" alt="" loading="lazy"><img class="winner-burst" src="assets/ui/winner-burst.png" alt=""><div class="winner-copy"><span class="pack-badge">${packBadge(p)}</span><div class="winner">${esc(g.title)}</div><div class="winner-sub">${playerLabel(g)} · ${g.tags.length?g.tags.map(t=>tagMeta[t].label).join(' · '):'Other interaction'}</div></div></div><div class="winner-actions">${launchButton(g.pack,packInfo(g.pack).kind==='standalone'?'Launch Survey Scramble':`Launch ${packBadge(packInfo(g.pack))}`)}<button class="ghost" data-spin-again>Spin again</button><button class="danger" data-remove-winner="${g.id}">Remove & spin again</button></div></div>`}
